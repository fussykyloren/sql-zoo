...
SELECT continent, name FROM world x
    WHERE LOWER(name) <= ALL(SELECT LOWER(name) FROM world y WHERE x.continent = y.continent);
-- 9. Find the continents where all countries have a population <= 25000000. 
-- Then find the names of the countries associated with these continents. 
-- Show name, continent and population.
SELECT name, continent, population FROM world w WHERE NOT EXISTS 
  (SELECT * FROM world nx WHERE nx.continent = w.continent AND nx.population > 25000000);
...
SELECT w1.name, w1.continent FROM world w1 WHERE w1.population > 
  (SELECT 3 * MAX(w2.population) FROM world w2 WHERE w2.continent = w1.continent AND w2.name <> w1.name);