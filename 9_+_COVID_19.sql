...
SELECT name, DAY(whn), confirmed - (LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY whn))
 FROM covid WHERE name = 'Italy' AND MONTH(whn) = 3 ORDER BY whn;
--  4. The data gathered are necessarily estimates and are inaccurate. 
...
--  5. ou can JOIN a table using DATE arithmetic. This will give different results if data is missing.
-- Show the number of new cases in Italy for each week - show Monday only.
...
-- 7. The query shown includes a JOIN t the world table so we can access the 
-- total population of each country and calculate infection rates (in cases per 100,000). 
