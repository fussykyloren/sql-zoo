...
SELECT id,vstadium, team1, team2 FROM game WHERE game.id = 1012;
...
SELECT game.team1, game.team2, goal.player FROM game JOIN goal ON (id=matchid)
...
SELECT game.mdate, eteam.teamname FROM eteam JOIN game ON (eteam.id=team1)
    WHERE coach = 'Fernando Santos';
-- 7. List the player for every goal scored in a game where the stadium was 'National Stadium, Warsaw'
SELECT goal.player FROM game JOIN goal ON (id=matchid)
...
SELECT mdate, team1, SUM(CASE WHEN teamid=team1 THEN 1 ELSE 0 END) score1, team2,
	SUM(CASE WHEN teamid=team2 THEN 1 ELSE 0 END), score2
		FROM game LEFT OUTER JOIN goal ON id = matchid GROUP BY id
